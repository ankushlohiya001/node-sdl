...
Video::Window::Window(const Napi::CallbackInfo &info) : ObjectWrap(info) {
  ...
  this->ref =
      SDL_CreateWindow(title.c_str(), pos_x, pos_y, width, height, flags);
}
...
void Video::Window::setTitle(const Napi::CallbackInfo &info,
...
void Video::Window::setIcon(const Napi::CallbackInfo &info) {
  ...
  SDL_Surface *surf = SDL_CreateRGBSurfaceWithFormatFrom(
      pixels, wid, hei, 32, wid * 4, SDL_PIXELFORMAT_ARGB8888);
...
void Video::Window::setPosition(const Napi::CallbackInfo &info,
                                const Napi::Value &value) {
  if (value.IsObject()) {
    Napi::Object pos = value.As<Napi::Object>();
    // native api call
    SDL_SetWindowPosition(this->ref, pos.Get("x").As<Napi::Number>(),
                          pos.Get("y").As<Napi::Number>());
  }
}
...
void Video::Window::setSize(const Napi::CallbackInfo &info,
                            const Napi::Value &value) {
  if (value.IsObject()) {
    Napi::Object size = value.As<Napi::Object>();
    // native api call
    SDL_SetWindowSize(this->ref, size.Get("w").As<Napi::Number>(),
                      size.Get("h").As<Napi::Number>());
  }
}
...
void Video::Window::setMinSize(const Napi::CallbackInfo &info,
                               const Napi::Value &value) {
  if (value.IsObject()) {
    Napi::Object size = value.As<Napi::Object>();
    // native api call
    SDL_SetWindowMinimumSize(this->ref, size.Get("w").As<Napi::Number>(),
                             size.Get("h").As<Napi::Number>());
  }
}
...
void Video::Window::setMaxSize(const Napi::CallbackInfo &info,
                               const Napi::Value &value) {
  if (value.IsObject()) {
    Napi::Object size = value.As<Napi::Object>();
    // native api call
    SDL_SetWindowMaximumSize(this->ref, size.Get("w").As<Napi::Number>(),
                             size.Get("h").As<Napi::Number>());
  }
}
/*-----------------------------------------------------------*/
...
void Video::Window::setResizable(const Napi::CallbackInfo &info,
  ...
  if (value.IsBoolean()) {
    // native api call
    SDL_SetWindowResizable(this->ref,
                           value.As<Napi::Boolean>() ? SDL_TRUE : SDL_FALSE);
  }
}

Napi::Value Video::Window::getFullscreen(const Napi::CallbackInfo &info) {
  bool isFull =
      (SDL_GetWindowFlags(this->ref) & SDL_WINDOW_FULLSCREEN_DESKTOP) != 0;
  return Napi::Boolean::New(info.Env(), isFull);
}

void Video::Window::setFullscreen(const Napi::CallbackInfo &info,
                                  const Napi::Value &value) {
  uint flag = 0;
...
Napi::Value Video::Window::getGrab(const Napi::CallbackInfo &info) {
  ...
  return Napi::Boolean::New(info.Env(),
                            SDL_GetWindowGrab(this->ref) == SDL_TRUE);
}

void Video::Window::setOpacity(const Napi::CallbackInfo &info,
                               const Napi::Value &value) {
  if (value.IsNumber()) {
    // native api call
...
void Video::Window::setBrightness(const Napi::CallbackInfo &info,
                                  const Napi::Value &value) {
  if (value.IsNumber()) {
    // native api call
...
void Video::Window::setBordered(const Napi::CallbackInfo &info,
                                const Napi::Value &value) {
  SDL_SetWindowBordered(this->ref,
                        value.As<Napi::Boolean>() ? SDL_TRUE : SDL_FALSE);
}
...
void Video::Window::setBlendMode(const Napi::CallbackInfo &info,
                                 const Napi::Value &value) {
...
void Video::Window::setCursor(const Napi::CallbackInfo &info,
                              const Napi::Value &value) {
...
Napi::FunctionReference *Video::Window::constructor =
...
Napi::Function Video::Window::GetClass(Napi::Env &env) {
  Napi::Function func = DefineClass(
      env, "Window",
      {
          ...
          InstanceAccessor<&Window::getPosition, &Window::setPosition>(
              "position"),
          ...
          InstanceAccessor<&Window::getFullscreen, &Window::setFullscreen>(
              "fullscreen"),
          InstanceAccessor<&Window::getResizable, &Window::setResizable>(
              "resizable"),
          InstanceAccessor<&Window::getGrab, &Window::setGrab>("grab"),
          InstanceAccessor<&Window::getBordered, &Window::setBordered>(
              "bordered"),
          InstanceAccessor<&Window::getOpacity, &Window::setOpacity>("opacity"),
          InstanceAccessor<&Window::getBlendMode, &Window::setBlendMode>(
              "blendMode"),
          InstanceAccessor<&Window::getBrightness, &Window::setBrightness>(
              "brightness"),
